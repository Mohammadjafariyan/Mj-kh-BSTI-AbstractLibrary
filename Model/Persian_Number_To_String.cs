    ...
    public static class PersianNumberToString
    {
        ...
        private static string Convert_STR(string INT, int Count)
        {
            ...
            if (Count == 0)
            {
                ...
                {
                    RET = GET_Number(3, Convert.ToDecimal(INT.Substring(0, 1)), " ") + GET_Number(1, Convert.ToDecimal(INT.Substring(2, 1)), "");
                }
                else
                {
                    string STR = GET_Number(0, Convert.ToDecimal(INT.Substring(2, 1)), "");
                    RET = GET_Number(3, Convert.ToDecimal(INT.Substring(0, 1)), GET_Number(2, Convert.ToDecimal(INT.Substring(1, 1)), "") + STR) + GET_Number(2, Convert.ToDecimal(INT.Substring(1, 1)), STR) + GET_Number(0, Convert.ToDecimal(INT.Substring(2, 1)), "");
                }
            }
            //هزار
            else if (Count == 1)
        ...
        private static string GET_Number(int Count, decimal Number, string VA)
        {
            string RET = "";
            ...
            if (Count == 0 || Count == 1)
            {
                ...
                return MySTR[Convert.ToInt16(Number)];
            }
            else if (Count == 2)
            {
                string[] MySTR = new string[10];
